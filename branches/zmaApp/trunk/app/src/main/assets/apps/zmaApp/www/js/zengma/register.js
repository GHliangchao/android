...
function plusReady(){
	var phone = mui("#phone")[0],
		...
		webv = plus.webview.currentWebview();
...
//	},false);
	...
	function cleanData(){
		...
		$("#password").val("");
		$("#cfmPassword").val("");
		$("#checkpoint").removeClass("checkbox_choosed");
	}
	
	$("#openProtocol")[0].addEventListener("tap", openProtocol, false);
	function openProtocol(){
		var dpage = plus.webview.getWebviewById("protocol.html");
		if(dpage==null){
			mui.preload({url: "protocol.html"});
		}else{
