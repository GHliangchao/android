...
function plusReady(){
	...
	window.addEventListener('restart', start, false);
    ...
    function start(){
    	...
    	// 使用openWindow方法打开该页面的时候，必须等页面显示了之后，才创建控件
    	// @since liujun 2018-11-21
//  	ws.removeEventListener('show', start, false);
    ...
    function onmarked(type, result) {
		...
		var page = plus.webview.getWebviewById("goods_scan_details.html");
		if (page == null) {
			mui.preload({url: "goods_scan_details.html", extras:{
				goodsCode:goodsCode
			}});
		} else {
...
//		}, 500);
    ...
    function checkNfCInfo(mes) {
    	...
    	// 1.判断是否是推点码的链接
    	if(mes.indexOf("?tcode=") == -1 && !conf.debug) {
    		showMsg(infoError);
    		return false;
    	}
