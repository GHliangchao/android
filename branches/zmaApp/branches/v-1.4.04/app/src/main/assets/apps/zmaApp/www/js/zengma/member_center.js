...
//						alert({content: "出错啦~"});
	...
	// + 1.4.04 地址簿列表 since liujun 2019.08.23
	$("#manageAddress")[0].addEventListener("tap", addressList, false);
	// + 1.4.04 已购列表 since liujun 2019.08.27
	$("#orderList")[0].addEventListener("tap", orderList, false);
	// + 1.4.04 快递列表 since wep 2019/08/279
	$("#expressList")[0].addEventListener("tap", expressList, false);
	...
	function addressList() {
		...
		if (dpage == null) {
			mui.preload({url: url, extras:{opType: 'M'}});
		} else {
			mui.fire(dpage, "preloading", {opType: "M"});
		}
	}
