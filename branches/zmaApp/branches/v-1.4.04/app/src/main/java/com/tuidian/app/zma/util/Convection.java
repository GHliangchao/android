...
public class Convection {
	...
	/**
	 * 10进制字符串转16进制字符串  例如："1610678288" ------>60010010
	 * **/
...
//		System.out.println("");
	...
	public static String certType2String(FTUserCertType certType){
		if(certType == FTUserCertType.FTO_CERTTYPE_RSA1024_MIX)
			return "RSA1024证书(用于签名/加解密)";
		if(certType == FTUserCertType.FTO_CERTTYPE_RSA1024_PLAIN)
			return "RSA1024证书(用于加解密)";
		if(certType == FTUserCertType.FTO_CERTTYPE_RSA1024_REEXAMINE)
			return "RSA1024证书(用于签名)";
		if(certType == FTUserCertType.FTO_CERTTYPE_RSA2048_MIX)
			return "RSA2048证书(用于签名/加解密)";
		if(certType == FTUserCertType.FTO_CERTTYPE_RSA2048_PLAIN)
			return "RSA2048证书(用于加解密)";
		if(certType == FTUserCertType.FTO_CERTTYPE_RSA2048_REEXAMINE)
			return "RSA2048证书(用于签名)";
		if(certType == FTUserCertType.FTO_CERTTYPE_SM2_MIX)
			return "SM2证书(用于签名/加解密)";
		if(certType == FTUserCertType.FTO_CERTTYPE_SM2_PLAIN)
			return "SM2证书(用于加解密)";
		if(certType == FTUserCertType.FTO_CERTTYPE_SM2_REEXAMINE)
			return "SM2证书(用于签名)";
		return null;
	}
}
