...
function plusReady() {
	var currentWebview = plus.webview.currentWebview(),
		lng = currentWebview.lng || "",
		lat = currentWebview.lat || "",
		zclockId = currentWebview.zclockId,
		confirm = $("#confirm"),
		cancel = $("#cancel"),
	...
	function initMap() {
		...
		plus.maps.Map.reverseGeocode(ptObj, {}, function(e){
			var address = e.address;
			var bubble = new plus.maps.Bubble(address);
			var showLng = ptObj.getLng().toFixed(6);
			if (ptObj.getLng() >= 0) {
				showLng = showLng + "°E";
			} else {
				showLng = Math.abs(showLng) + "°W";
			}
			var showLat = ptObj.getLat().toFixed(6);
			if (ptObj.getLat() >= 0) {
				showLat = showLat + "°N";
			} else {
