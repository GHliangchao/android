...
// NFC test
function nfcRead(self){
	var btn = $(self);
	var status = $("#nfc_status");
	var options= {cardId: getAddress(), format:"txt"};
...
function nfcWrite(self){
	var btn = $(self);
	var status = $("#nfc_status");
	var options= {cardId: getAddress(), format:"txt"};

	options.data  = "操作人员：推点科技M+G组\n";
	options.data += "操作时间："+new Date().Format("yyyy-MM-dd HH:mm:ss")+"\n";
