...
public class BleLockFeatureImpl extends StandardFeature {
	...
	final static String NFC_FMT_TXT= "txt", NFC_FMT_HEX   = "hex";
	...
	public void nfcRead(final IWebview webview, final JSONArray params) {
		...
		if(isSimulated(cardId)){
			...
			final String data;
			if(NFC_FMT_HEX.equalsIgnoreCase(format)){
				data = ByteUtil.dumphex(buf);
			}else{
				try {
					data = new String(buf, Packet.ENCODING);
	...
	public void nfcWrite(final IWebview webview, final JSONArray params) {
		...
		if(isSimulated(cardId)){
			final byte[] buf;
			if(NFC_FMT_HEX.equalsIgnoreCase(format)){
				buf = ByteUtil.parsehex(data);
			}else{
				try {
					buf = data.getBytes(Packet.ENCODING);
				} catch (final UnsupportedEncodingException e) {
					throw new RuntimeException(e);
				}
			}
	...
	private final String nfcParseArgFormat(IWebview webview, String callback, JSONObject options){
		String dataf = options.optString(KEY_NFC_FMT, NFC_FMT_TXT);
		if(dataf.length() == 0){
			dataf = NFC_FMT_TXT;
		}
		final String format = dataf;
		if(!NFC_FMT_TXT.equalsIgnoreCase(format) && !NFC_FMT_HEX.equalsIgnoreCase(format)){
			error(webview, callback, Consts.ER_ERROR, "Arguments error: format must be "+
											             NFC_FMT_TXT+" or "+NFC_FMT_HEX);
			return null;
		}
		return format;
	}
	...
	public void onNewIntent(final Intent intent){
		debug(TAG, "onNewIntent(): intent = %s", intent);
