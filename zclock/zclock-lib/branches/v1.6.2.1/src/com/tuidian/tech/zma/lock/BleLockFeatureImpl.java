	...
	private final BleNotifyResponse notifyResponse = new BleNotifyResponse() {
		...
		final void newCommandResponse(final byte[] value) {
			...
			case CMD_HANDS:
				...
				break;
			case CMD_RCODE:
				commandResponse = new ReadZcodeResponse(cmd.encrypted);
				break;
			case CMD_WCODE:
			case CMD_OLOCK:
			case CMD_CLOCK:
			...
			case CMD_UPGSWT:
			case CMD_MODKEY:
				commandResponse = new Response(cmd.encrypted, cmd.id);
				break;
			case CMD_LOCAT:
				commandResponse = new LocateResponse(cmd.encrypted, cmd.id);
				break;
			default:
		...
		final void onCommandCompleted(final Packet response) {
			final JSONObject json = new JSONObject();
			try{
			    // @since liujun 2018-03-21
			    // 回复包不用透传到服务器解密，直接在android库里解析得出操作结果
