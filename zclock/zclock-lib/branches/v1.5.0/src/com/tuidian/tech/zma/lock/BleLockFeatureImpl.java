...
import static com.tuidian.tech.zma.lock.util.Logger.*;
import static com.tuidian.tech.zma.lock.protocol.Packet.*;
...
public class BleLockFeatureImpl extends StandardFeature {
	...
	// Protocol debug
	private final SimulatedLock simLock = new SimulatedLock();
	...
	public void handshake(final IWebview webview, final JSONArray params){
		...
		init(webview, params.optString(0));
		if(checkConnect()){
			return;
		}
		int request = 0;
	...
	public void wzcode(final IWebview webview, final JSONArray params) {
		...
		try{
			if(encrypted){
				cmd.setLength(zcode.getInt("plainLength"));
				cmd.setBody(zcode.getString("packetBody"));
			}else{
	...
	public void switchUpgrade(final IWebview webview, final JSONArray params) {
		...
		try{
			...
			if(encrypted){
				cmd.setLength(options.getInt("plainLength"));
				cmd.setBody(options.getString("packetBody"));
			}else{
	...
	private final String getZcodeSign(final Packet cmd, final JSONObject zcode, final String keyName){
		...
		final String sign = zcode.optString("zcSign", zcsDefault);
		if(sign.length() == 0){
			return zcsDefault;
		}
		return sign;
	...
	private void init(final IWebview webview, final String callback){
		this.webview  = webview;
	...
	private final BleNotifyResponse notifyResponse = new BleNotifyResponse() {
        ...
        public void onResponse(final int code) {
            ...
            if (code == REQUEST_SUCCESS || isSimulated()) {
            	...
            	onConnectionCompleted("连接成功");
            } else {
            	debugf(TAG, "onResponse(): request failed");
            	try{
            		error(webview, connectCb, Consts.ER_ERROR, "启用通知出错");
            	}finally{
            		cleanupConnection();
            	}
            }
		...
		final void readResponse(final Packet packet, final byte[] value, final boolean init) {
        	...
        	final int len = 2 + packet.getRealLength() + 1;
        	debugf(TAG, "readResponse(): pos = %d <-> len = %d, init = %s", pos, len, init);
			if(pos >= len){
		...
		final void newCommandResponse(final byte[] value) {
			...
			case CMD_HANDS:
				...
				break;
			case CMD_RCODE:
				commandResponse = new ReadZcodeResponse(command.encrypted);
				break;
			case CMD_WCODE:
			case CMD_OLOCK:
			case CMD_CLOCK:
			...
			case CMD_UPGSWT:
			case CMD_MODKEY:
				commandResponse = new ResultResponse(command.encrypted, command.id);
				break;
			case CMD_LOCAT:
				commandResponse = new LocateResponse(command.encrypted, command.id);
				break;
			default:
		...
		final void onResponseCompleted(final Packet response) {
	    	...
	    	if(response.checkCRC() == false){
	    		...
	    		if(commandCb != null){
	    			safeError(Consts.ER_ERROR, "协议错误");
	    			return;
	    		}
	    	}
		...
		final void onCommandCompleted(final Packet response) {
			...
			try{
				...
				case CMD_HANDS:
					...
					String error = null;
					if((STATUS_FAULTED & status) != 0){
						error = "锁出现故障";
					}
					if((STATUS_VINCOMP & status) != 0){
						error = "协议不兼容";
					}
					if(error != null){
				...
				case CMD_UPGSWT:
				case CMD_MODKEY:
					final ResultResponse resRsp = (ResultResponse)response;
